...
def run_all_rules():
    ...
    count = c.fetchone()[0]
    if count > 0:
        alerts.append(("🔴 Rule 1", f"{count} vaccination(s) are overdue!"))
    ...
    if deaths > 5:
        alerts.append(("🔴 Rule 2", f"High mortality: {deaths} deaths in last 7 days!"))
    ...
    c.execute("SELECT SUM(cost) FROM feed_expenses WHERE date >= ?", (this_week_start,))
    ...
    if last_week > 0 and this_week > last_week * 1.3:
        alerts.append(("🟡 Rule 3", f"Feed cost spike! This week: {this_week:.0f} vs last week: {last_week:.0f}"))

    # Rule 4: Low inventory
    c.execute("SELECT COUNT(*) FROM inventory WHERE quantity < min_threshold")
    low = c.fetchone()[0]
    if low > 0:
        alerts.append(("🟡 Rule 4", f"{low} inventory item(s) below minimum threshold!"))

    # Rule 5: Monthly loss (expense > income)
    ...
    if expense > income:
        alerts.append(("🔴 Rule 5", f"Monthly LOSS! Income: {income:.0f} | Expense: {expense:.0f}"))
